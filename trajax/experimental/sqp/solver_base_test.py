...
class TrajprobTest(absltest.TestCase):
    ...
    def setUp(self):
        ...
        self.prob = solver_base.TrajectoryOptimizationSolver(
            n, m, T, dynamics, cost, control_bounds, state_constraint, s1_indices
        )
        ...
        self.X = self.prob._vec_wrap(
            optimizers.rollout(self.prob._dynamics, self.U, self.x0)
        )
    ...
    def testGetQPParams(self):
        ...
        self.assertEqual(constraint_vals[1].shape, (self.T, self.prob._n_cu, self.m))
        self.assertEqual(
            constraint_vals[3].shape, (self.T + 1, self.prob._n_cx, self.n)
        )
    ...
    def testAdjointHess(self):
        ...
        self.assertEqual(
            hess_params[0].shape, (self.T, self.n + self.m, self.n + self.m)
        )
        self.assertEqual(hess_params[1].shape, (self.T + 1, self.n, self.n))
    ...
    def testMerit(self):
        ...
        merit = self.prob._merit_fn(
            self.U, self.X, Yu, Yx, Su, Sx, Rho, self.prob_params
        )
    ...
    def testLQRCompute(self):
        ...
        dyn_params, _, _ = self.prob._get_QP_params(self.U, self.X, self.prob_params)
        gains = self.prob._get_LQR_control_gains(
            self.U, self.X, dyn_params, 1.0, self.prob_params[0]
        )
