...
cvxpy_solvers = {"ecos": cp.ECOS, "cvxopt": cp.CVXOPT}
...
class TrajQP(abc.ABC):
    ...
    def __init__(self, n: int, m: int, T: int, n_cu: int, n_cx: int, **qp_options):
    ...
    def reset_soln(self):
        ...
        self.soln = jax.tree.map(jnp.zeros_like, self.soln)
        return self.soln
...
class TrajQPcvx(TrajQP):
    ...
    def __init__(
        self, n: int, m: int, T: int, n_cu: int, n_cx: int, solver: str, **qp_options
    ):
        """Initialize the problem."""
    ...
    def _create_prob(self):
        ...
        for j in range(T):
            ...
            cost += (
                (1 / 2) * cp.quad_form(dz, self._Z[j])
                + self._q[j] @ dx
                + self._r[j] @ self._dU[j]
            )

            self._constr_ineq_u += [self._Cu[j] + self._Ju[j] @ self._dU[j] >= 0.0]
            if j > 0:
                self._constr_ineq_x += [self._Cx[j] + self._Jx[j] @ dx >= 0.0]
            self._constr_dyn += [
                self._dX[j] == self._A[j] @ dx + self._B[j] @ self._dU[j]
            ]

        cost += (1 / 2) * cp.quad_form(self._dX[-1], self._ZT) + self._q[-1] @ self._dX[
            -1
        ]
        self._constr_ineq_x += [self._Cx[-1] + self._Jx[T] @ self._dX[-1] >= 0.0]
    ...
    def solve(self, prev_soln, *updated_params):
        ...
        if self._prob.status not in ("infeasible", "unbounded"):
            ...
            dX_d = jnp.vstack((jnp.zeros((self._n,)), device_put(self._dX.value)))
            Yu = jnp.vstack(
...
class TrajQPalqr(TrajQP):
    ...
    def _qp_cost(self, dx, du, k, Z, ZT, q, r):
        ...
        stage_cost = (
            0.5 * jnp.vdot(dz, Z[k] @ dz) + jnp.vdot(q[k], dx) + jnp.vdot(r[k], du)
        )
    ...
    def _qp_cons(self, dx, du, k, Cu, Ju, Cx, Jx):
        ...
        state_cons = jnp.where(k == 0, -jnp.ones(self._n_cx), -(Cx[k] + Jx[k] @ dx))
        control_cons = jnp.where(
            k == self._T, -jnp.ones(self._n_cu), -(Cu[k] + Ju[k] @ du)
        )
    ...
    def solve(self, dU_0, *updated_params):
        ...
        results = optimizers.constrained_ilqr(
            cost, dyn, dx_0, dU_0, inequality_constraint=ineq_cons, **self._options
        )
