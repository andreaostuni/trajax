...
# Ensure JAX loads with float64 support before importing jax.
...
def pendulum(state, action, t, params=(1.0, 1.0, 9.81)):
    ...
    m, l, g = params
    return np.array(
        [theta_dot, (np.squeeze(action) - m * g * l * np.sin(theta)) / (m * l * l)]
    )
...
class OptimizersTest(parameterized.TestCase):
    ...
    def setUp(self):
        ...
        def cost(x, u, t, params):
            ...
            return params[0] * 0.5 * np.matmul(z.T, np.matmul(self.H[t], z)) + params[
                1
            ] * np.dot(self.h[t], z)
    ...
    def testLinearize(self):
        ...
        def grad(x, u, t):
            return (
                params[0] * np.matmul(self.H[t], np.concatenate((x, u)))
                + params[1] * self.h[t]
            )
    ...
    def testGradient(self):
        ...
        def cost(x, u, t, params):
            ...
            stagewise_cost = 0.5 * params[1] * np.dot(delta, delta) + 0.5 * params[
                2
            ] * np.dot(u, u)
        ...
        objective = functools.partial(
            optimizers.objective, functools.partial(cost, params=params_cost), dynamics
        )
    ...
    def testCustomVJP(self, vjp_method, vjp_options):
        ...
        def cost(params, state, action, t):
            ...
            return np.where(
                t == horizon, final_weight * state_cost, state_cost + action_cost
            )
        ...
        def train_loss(params):
            ...
            return np.sum(np.square(train_xs - proposed_xs)) + np.sum(
                np.square(train_us - proposed_us)
            )
    ...
    def testCustomVJPPyTree(self, vjp_method, vjp_options):
        ...
        def pytree_cost(params, state, action, t):
            ...
            return cost(
                np.array([final_weight, stage_weight, action_weight]), state, action, t
            )

        def multiparam_cost(
            final_weight, stage_weight, action_weight, state, action, t
        ):
            return cost(
                np.array([final_weight, stage_weight, action_weight]), state, action, t
            )
        ...
        def pytree_loss(params):
            u0 = np.zeros((horizon, 1))
            xs, us, *_ = optimizers.ilqr(
                functools.partial(pytree_cost, params),
                dynamics,
                x0,
                u0,
                vjp_method=vjp_method,
        ...
        def multiparam_loss(final_weight, stage_weight, action_weight):
            ...
            xs, us, *_ = optimizers.ilqr(
                functools.partial(
                    multiparam_cost, final_weight, stage_weight, action_weight
                ),
                dynamics,
                x0,
                u0,
        ...
        assert np.allclose(
            ...
            np.array(
                [
                    grad_pytree_params["final_weight"],
        ...
        for idx, grad_param in enumerate(grad_params):
            assert np.allclose(
                grad_param, jax.jacobian(multiparam_loss, argnums=idx)(*params)
            )
    ...
    def testCustomVJPConsistency(self):
        ...
        jacs = [
            make_jac_function(vjp_method, vjp_options)(params)
            for vjp_method, vjp_options in zip(vjp_methods, vjp_options_per_method)
        ]

        def pytree_allclose(a, b, *args, **kwargs):
            return jax.flatten_util.ravel_pytree(
                jax.tree.map(lambda x, y: np.allclose(x, y, *args, **kwargs), a, b)
            )[0].all()
    ...
    def testGradientOptimization(self, vjp_method, vjp_options):
        ...
        X_true, U_true = make_optimal_traj(A_true, B_true, Q_true, R_true, x0_true, T)
        ...
        def opt_params(
            ...
            param_final, losses = jax.lax.scan(
                scan_fn, param_init, xs=np.arange(num_iters)
            )
    ...
    def testConstrainedCarSolve(self):
        ...
        def inequality_constraint(x, u, t):
            def obstacles(x):
                return np.array(
                    [
                        obs1["r"]
                        - np.sqrt(
                            (x[0] - obs1["px"]) ** 2.0 + (x[1] - obs1["py"]) ** 2.0
                        )
                    ]
                )
            ...
            return np.where(
                t == T,
                np.concatenate((np.zeros(2 * m), obstacles(x))),
        ...
        sol = optimizers.constrained_ilqr(
            functools.partial(cost, **cost_args),
            dynamics,
            x0,
            U,
            equality_constraint=equality_constraint,
        ...
        self.assertFalse(np.any(U < u_lower - constraints_threshold))
        self.assertFalse(np.any(U > u_upper + constraints_threshold))
        self.assertLess(np.linalg.norm(X[-1] - goal, ord=np.inf), constraints_threshold)
